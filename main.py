...
self.root.geometry("550x650") # Размерокна
...
self.load_all_tasks_data()     # Загрузкаданныхзадач
self.load_task_history()       # Загрузкаистории
self.update_task_pool_and_filters() # Обновляемпулзадачиустанавливаемфильтры

self.create_widgets()          # Созданиевиджетов GUI

self.refresh_history_display() # Отображениеисториипризапуске

defload_all_tasks_data(self):
"""
...
print(f"Ошибкапризагрузкезадачиз '{TASKS_DATA_FILE}': {e}. Используютсяначальныезадачи.")
self.all_tasks_data = INITIAL_TASKS_DATA
...
print(f"Предупреждение: Некорректный формат файла '{TASK_HISTORY_FILE}'. Историябудетпустой.")
self.task_history = []
            except (json.JSONDecodeError, IOError) as e:
print(f"Ошибкапризагрузкеисториизадачиз '{TASK_HISTORY_FILE}': {e}. Историябудетпустой.")
self.task_history = []
        else:
self.task_history = [] # Еслифайланет, историяпуста

defsave_task_history(self):
"""
...
defupdate_task_pool_and_filters(self):
"""
...
self.task_types = ["Все"] + unique_types # Добавляемопцию"Все"

        # Обновляем значения для выпадающего списка фильтров
ifhasattr(self, 'filter_combobox'): # Проверяем, существует ли уже виджет
...
self.apply_filter() # Применяемтекущийфильтрдляустановкиfiltered_task_pool

defapply_filter(self, event=None):
"""
        Применяет выбранный тип задачи для фильтрации общего пула задач.
        Обновляетself.filtered_task_pool.
        """
...
messagebox.showwarning("Нетзадач", f"Повыбранномутипу '{current_filter_type}' нетзадач.")
self.selected_type.set("Все") # Сбрасываем на "Все"
self.filtered_task_pool = self.all_tasks_data # Возвращаемполныйпул

defgenerate_task(self):
        """
...
selected_task_string = f"[{selected_task_dict['type']}] {selected_task_dict['task']}"
        ...
        # Добавляемзадачувисторию
self.task_history.append(selected_task_string)

        # Обновляемисториювфайле
self.save_task_history()

        # ОбновляемотображениеисториивListbox
self.refresh_history_display()

# Показываем пользователю сгенерированную задачу (можно в отдельном окне или метке)
messagebox.showinfo("Вашазадача:", selected_task_string)

defrefresh_history_display(self):
"""
...
tk.Label(filter_frame, text="Фильтрпотипу:", font=("Arial", 10)).grid(row=0, column=0, sticky="w", padx=5, pady=5)
...
self.filter_combobox.bind("<<ComboboxSelected>>", self.apply_filter) # Событиепривыбореэлемента

self.generate_button = tk.Button(
filter_frame,
            text="Сгенерироватьзадачу",
            command=self.generate_task,
bg="#4CAF50", # Зеленыйцвет
fg="white",
            font=("Arial", 10, "bold"),
            relief=tk.RAISED,
bd=2
...
self.generate_button.grid(row=0, column=2, padx=15, pady=5)
        ...
        # --- Фреймдляотображенияистории ---
history_frame = tk.Frame(self.root, padx=10, pady=10, relief=tk.GROOVE, borderwidth=2)
history_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

tk.Label(history_frame, text="Историясгенерированныхзадач:", font=("Arial", 12, "bold")).pack(pady=5)

        # Создаем Scrollbar дляListbox
...
self.scrollbar.config(command=self.history_listbox.yview) # Настраиваемскроллбар
